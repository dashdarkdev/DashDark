...
uint256 hashGenesisBlock("0x000009e173c3b4d16d8663c0cd159d73345579542440c8e1285993a7bfa2b7fe"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DashDark: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DashDark Signed Message:\n";
...
{
    // DashDark: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 0.1 * 60 * 60; // DashDark: 10 minutes
static const int64 nTargetSpacing = 1 * 60; // DashDark: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DashDark: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, dashdark - DarkGravity v3, written by Evan Duffield - evan@dashdark.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // DashDark: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // DashDark: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Zcash is at a low from 45 dollars 28/Jan/17.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xfb3066352ee52598ccc2820da9114154777e3b515511d4e1083e028c6e32fe55"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DashDarkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DashDarkMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DashDarkMiner(CWallet *pwallet)
{
    LogPrintf("DashDarkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dashdark-miner");
    ...
    try { loop {
        ...
        LogPrintf("Running DashDarkMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("DashDarkMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DashDarkMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - DARKCOIN **********
    Copyright 2014, DashDark Developers - ( evan@dashdark.io )
*/
